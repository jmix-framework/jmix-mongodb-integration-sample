...
[[defining-visitlog-dto]]
=== Defining the VisitLog DTO Entity

The `VisitLog` entity is used in the Jmix UI for displaying and managing visit log data. It does not interact directly with MongoDB, serving instead as a clean representation of data for the UI components. It is implemented as a xref:data-model/entities.adoc[DTO Entity].

.VisitLog.java
[source,properties]
----
include::example$src/main/java/io/jmix/petclinic/visit/log/VisitLog.java[]
----

[[defining-visitlogdocument-persistence]]
...
include::path/to/VisitLogDocument.java[]

In this setup:

* The `VisitLogDocument` entity handles database interactions, with fields like `visitId` used as identifiers for the associated `Visit` entity, rather than full object references.
* The `VisitLog` DTO provides a UI-facing model, with fields like `visit` representing direct associations that are useful for display and interaction in the Jmix UI.
